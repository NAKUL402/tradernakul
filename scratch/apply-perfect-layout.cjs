...
let original = fs.readFileSync('src/routes/reports.tsx', 'utf-8');
const anchor = '<div className="space-y-5">';
const startIdx = original.indexOf(anchor);

if (startIdx !== -1) {
    let newContent = original.substring(0, startIdx) + code;
    
    // 2. Add toast import
    if (!newContent.includes('import { toast }')) {
        newContent = newContent.replace('import { cn } from "@/lib/utils";', 'import { cn } from "@/lib/utils";\\nimport { toast } from "sonner";');
    }
    
    // 3. Fix tables to be scrollable and nowrap
    newContent = newContent.replace(
      '<table className="w-full min-w-[420px] text-sm">',
      '<table className="w-full min-w-[420px] text-sm whitespace-nowrap">'
    );
    newContent = newContent.replace(
      '<table className="w-full min-w-[420px] text-sm">', // Need to do it twice because there are 2 tables
      '<table className="w-full min-w-[420px] text-sm whitespace-nowrap">'
    );
    
    // Fix toast syntax error immediately
    newContent = newContent.replace('import { cn } from "@/lib/utils";\\\\nimport { toast } from "sonner";', 'import { cn } from "@/lib/utils";\\nimport { toast } from "sonner";');
    
    // Find precise targets using indexOf
    
    const targetFilter = '          <div className="grid size-8 place-items-center rounded-xl bg-primary/10 text-primary border border-primary/20">\\n            <SlidersHorizontal className="size-4" />\\n          </div>';
    
    const newFilter = '          <button type="button" onClick={() => toast.info("Advanced filtering coming soon")} className="grid size-9 place-items-center rounded-lg border border-zinc-800 bg-[#0c0c0e] text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer">\\n            <SlidersHorizontal className="size-4" />\\n          </button>';
    
    const targetDate = '          <div className="relative flex items-center">\\n            <Calendar className="pointer-events-none absolute left-3 size-3.5 text-muted-foreground" />\\n            <select\\n              aria-label="Select date range"\\n              value={dateRange}\\n              onChange={(e) => setDateRange(e.target.value as typeof dateRange)}\\n              className="cursor-pointer rounded-xl border border-border bg-card pl-9 pr-8 py-2 text-xs font-semibold text-foreground outline-none transition focus:border-primary focus:ring-1 focus:ring-primary/40"\\n            >\\n              <option value="all">All Time</option>\\n              <option value="7d">Last 7 Days</option>\\n              <option value="30d">Last 30 Days</option>\\n              <option value="90d">Last 90 Days</option>\\n              <option value="this-month">This Month</option>\\n              <option value="this-week">This Week</option>\\n            </select>\\n          </div>';
    
    const newDate = '          <div className="relative flex items-center shadow-lg">\\n            <Calendar className="pointer-events-none absolute left-3 size-3.5 text-slate-400" />\\n            <select\\n              aria-label="Select date range"\\n              value={dateRange}\\n              onChange={(e) => setDateRange(e.target.value as typeof dateRange)}\\n              className="cursor-pointer rounded-full border border-zinc-700/80 bg-[#15151a] pl-9 pr-8 py-2 text-xs font-semibold text-zinc-200 outline-none transition-all hover:border-zinc-500 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 appearance-none shadow-md"\\n            >\\n              <option value="all">All Time</option>\\n              <option value="7d">Last 7 Days</option>\\n              <option value="30d">Last 30 Days</option>\\n              <option value="90d">Last 90 Days</option>\\n              <option value="this-month">This Month</option>\\n              <option value="this-week">This Week</option>\\n            </select>\\n            <div className="pointer-events-none absolute right-3">\\n               <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">\\n                 <path d="M1 1L5 5L9 1" stroke="#a1a1aa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>\\n               </svg>\\n            </div>\\n          </div>';
    
    if (newContent.includes(targetFilter)) {
        newContent = newContent.replace(targetFilter, newFilter);
    } else {
        console.log("Could not find targetFilter");
    }
    
    if (newContent.includes(targetDate)) {
        newContent = newContent.replace(targetDate, newDate);
    } else {
        console.log("Could not find targetDate");
    }
    
    fs.writeFileSync('src/routes/reports.tsx', newContent);
    console.log("Successfully patched reports.tsx fully");
    
} else {
    console.log("Error: Could not find anchor tag to inject layout.");
}
