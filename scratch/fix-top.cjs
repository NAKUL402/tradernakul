...
let content = fs.readFileSync('src/routes/reports.tsx', 'utf-8');
...
// 2. Fix Filter Button
const old_filter = \`          <div className="grid size-8 place-items-center rounded-xl bg-primary/10 text-primary border border-primary/20">
            <SlidersHorizontal className="size-4" />
          </div>\`;
const new_filter = \`          <button type="button" onClick={() => toast.info("Advanced filtering coming soon")} className="grid size-9 place-items-center rounded-lg border border-zinc-800 bg-[#0c0c0e] text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer">
            <SlidersHorizontal className="size-4" />
          </button>\`;
content = content.replace(old_filter, new_filter);

// 3. Fix Date Range
const old_date = \`          <div className="relative flex items-center">
...
const new_date = \`          <div className="relative flex items-center shadow-lg">
...
fs.writeFileSync('src/routes/reports.tsx', content, 'utf-8');
