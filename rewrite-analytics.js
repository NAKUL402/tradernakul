        ...
        <tbody className=\"divide-y divide-zinc-800/40\">
          {rows.map((row) => (
            <tr key={row.name} className=\"transition-colors hover:bg-zinc-800/20 group\">
              ...
              <td className={\py-2.5 text-right font-semibold pr-2 \\}>
                {money(row.pnl)}
              </td>
            </tr>
          ))}
...
function Analytics() {
  ...
  const byDow = groupStats(
    userTrades,
    (t) => DOW[new Date(\\T00:00:00Z\).getUTCDay()]!,
  ).map((g) => ({ ...g, label: g.name }));
  const byHour = groupStats(userTrades, (t) => \\:00\)
    .sort((a, b) => (a.name < b.name ? -1 : 1))
    .map((g) => ({ ...g, label: g.name }));
  const months = monthly(userTrades);
  const risk = groupStats(userTrades, (t) => \\%\)
          ...
          <div className=\"grid gap-6 lg:grid-cols-[1fr_1.5fr]\">
            ...
            <div className=\"space-y-4\">
              ...
              <div className=\"grid grid-cols-2 gap-3\">
                ...
                <MetricCard icon={<TrendingDown className=\"size-4\" />} label=\"Max Drawdown\" value={\\%\} tone=\"negative\" />
...
fs.writeFileSync('src/routes/analytics.tsx', newContent);
console.log('Successfully wrote to src/routes/analytics.tsx');
