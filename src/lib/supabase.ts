    ...
    getUser: async () => {
      if (isDevTestMode) {
        ...
        return {
          data: {
            user: {
              id: "dev-test-owner-id",
              email: "test-owner@local.test",
              user_metadata: mockMeta,
            },
          },
          error: null,
        };
      ...
      if (isDevTestMode) {
        if (attributes.data) {
          ...
          localStorage.setItem(
            "mock_user_metadata",
            JSON.stringify({ ...existing, ...attributes.data }),
          );
        }
    ...
    ? {
        from: (bucket: string) => ({
          ...
          getPublicUrl: (path: string) => {
            ...
            return {
              data: { publicUrl: dataUrl || `https://mock-storage.local/${bucket}/${path}` },
            };
          },
      ...
      const seedData = (table: string) => {
        ...
        if (table === "audit_logs") {
          return [
            {
              id: "log-1",
              admin_email: "test-owner@local.test",
              action: "test",
              target_type: "system",
              target_id: "1",
              previous_state: "{}",
      ...
      const createChain = (currentData: any[]) => {
        const chain: any = {
          ...
          eq: (col: string, val: any) => {
            chain._data = chain._data.filter((r: any) => r[col] === val);
            return chain;
          },
          ...
          order: (col: string, { ascending = true } = {}) => {
            chain._data.sort((a: any, b: any) =>
              ascending ? (a[col] > b[col] ? 1 : -1) : a[col] < b[col] ? 1 : -1,
            );
            return chain;
          },
          limit: (num: number) => {
            chain._data = chain._data.slice(0, num);
            return chain;
          },
          single: () => {
            if (chain._data.length === 0)
          ...
          maybeSingle: () => {
            return Promise.resolve({
              data: chain._data.length > 0 ? chain._data[0] : null,
              error: null,
            });
          },
          ...
          const updateChain: any = {
            ...
            in: (col: string, vals: any[]) => {
              tableData = tableData.map((r: any) =>
                vals.includes(r[col]) ? { ...r, ...record } : r,
              );
              persist();
              return Promise.resolve({ data: record, error: null });
            },
