  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Analytics — Edge Journal" },
      {
        ...
        content:
          "Interactive charts for pair, session and setup performance plus drawdown and risk analysis.",
      },
    ],
  }),
  component: Analytics,
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.name} className="border-b border-border/40 transition hover:bg-muted/10">
              ...
              <td
                className={`py-2.5 text-right font-semibold ${row.pnl >= 0 ? "text-success" : "text-destructive"}`}
              >
                {money(row.pnl)}
              </td>
            </tr>
  ...
  return (
    <AppShell title="Analytics" subtitle="Advanced performance breakdown">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Panel title="Pair-wise Performance">
          <BarsChart data={byPair.map((p) => ({ ...p, label: p.name }))} />
          <Table rows={byPair} />
        </Panel>
        <Panel title="Session-wise Performance">
          <BarsChart data={bySession.map((p) => ({ ...p, label: p.name }))} />
          <Table rows={bySession} />
        </Panel>
        <Panel title="Setup-wise Performance">
          <Table rows={bySetup} />
        </Panel>
        <Panel title="Day of Week">
          <BarsChart data={byDow} />
        </Panel>
        <Panel title="Time of Day (entry hour)">
        ...
        <Panel title="Drawdown Analysis" className="lg:col-span-2">
