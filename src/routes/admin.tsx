...
import {
  ...
  Wrench, Send, Plus, Edit3, SlidersHorizontal, RefreshCw, Play, Key, Brain,
  Lock, Unlock, Mail, FileText, LayoutDashboard, Sliders, AlertTriangle
} from "lucide-react";
...
function AdminPage() {
  const { user, isAdmin, isOwner, isLoading: isAuthLoading, siteSettings, refreshSettings } = useAuth();
  ...
  // Users Filter State
  const [userFilter, setUserFilter] = useState<"all" | "pending" | "approved" | "rejected" | "suspended">("all");
  ...
  const logAdminAction = async (action: string, targetType: string, targetId: string | null, prev: any, current: any) => {
    try {
  ...
  const handleSavePlatformSettings = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        await fetchStats();
        await logAdminAction("update_platform_settings", "site_settings", "1", prevState, nextState);
      }
    } catch (err: unknown) {
  ...
  const handleUpdateUserStatus = async (targetId: string, newStatus: Profile["status"]) => {
    ...
    try {
      ...
      await fetchStats();
      await logAdminAction(`change_user_status_${newStatus}`, "profiles", targetId, prevState, nextState);
  ...
  const sidebarItems = [
    ...
    { id: "approvals", label: "Pending Approvals", icon: Clock, badge: usersList.filter(u => u.status === "pending").length, badgeColor: "bg-accent/20 text-accent border-accent/30" },
    ...
    { id: "emergency", label: "Emergency Center", icon: ShieldAlert, badge: siteSettings?.maintenance_mode ? "ON" : undefined, badgeColor: "bg-red-500/20 text-red-300 border-red-500/30 border animate-pulse" }
  ] as const;
    ...
    return (
      <AppShell title="Admin Control Center">
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground font-semibold">Validating session permissions…</p>
        </div>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell title="Login Required" subtitle="Authentication mandatory">
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">Please log in using the owner credentials to access these controls.</p>
          <a href="/login" className="mt-6 rounded-xl bg-gradient-to-r from-primary to-accent px-6 py-2.5 text-sm font-semibold text-primary-foreground transition hover:opacity-90 glow-primary active:scale-95">
            Log In normally
          </a>
        </div>
      </AppShell>
    ...
    return (
      <AppShell title="Access Denied" subtitle="Unauthorized access attempt logged">
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <h1 className="mt-6 font-display text-2xl font-bold text-foreground">403 Forbidden Access</h1>
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">Only authorized administrators are permitted to view the Control Center.</p>
          <button onClick={() => navigate({ to: "/" })} className="mt-6 rounded-xl border border-border bg-card/60 px-5 py-2.5 text-sm font-medium hover:bg-card transition">
            Back to Dashboard
          </button>
        </div>
      </AppShell>
  ...
  return (
    <AppShell title="Admin Control Center" subtitle={`Logged in as ${isOwner ? "Owner Administrator" : "Administrator"}`}>
      ...
      <div className="mt-4 flex flex-col gap-6 lg:flex-row">
        ...
        <aside className="w-full shrink-0 lg:w-64">
          <div className="glass flex flex-col gap-1 rounded-2xl border border-border p-3.5 sticky top-20">
            <p className="px-2 pb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">System Navigation</p>
            <nav className="flex flex-row lg:flex-col overflow-x-auto lg:overflow-x-visible gap-1 pb-2 lg:pb-0 scrollbar-none">
              {sidebarItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <IconComponent className={`size-4 ${isSelected ? "text-primary" : "text-muted-foreground"}`} />
                    <span className="flex-1 text-left">{item.label}</span>
                    {item.badge !== undefined && item.badge > 0 && (
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${item.badgeColor || "bg-muted text-foreground"}`}>
                        {item.badge}
                      </span>
                    )}
                  </button>
        ...
        <main className="flex-1 overflow-hidden min-h-[60vh] flex flex-col">
          ...
          {activeTab === "overview" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <StatCard label="Total Registered Users" value={stats ? String(stats.total_users) : String(usersList.length)} icon={<Users className="size-4" />} />
                <StatCard label="Awaiting Approval" value={stats ? String(stats.pending_users) : String(usersList.filter(u => u.status === "pending").length)} icon={<Clock className="size-4" />} accent="accent" />
                <StatCard label="Total Logged Trades" value={stats ? String(stats.total_trades) : String(totalDbTrades)} icon={<Activity className="size-4" />} accent="success" />
                <StatCard label="AI Chat Queries" value={stats ? String(stats.total_ai_chats) : "Data unavailable"} icon={<Brain className="size-4" />} />
              </div>
              ...
              {siteSettings?.maintenance_mode && (
                <div className="rounded-2xl border border-red-500/40 bg-red-500/10 p-5 text-red-200 flex gap-4 items-start relative overflow-hidden">
                  ...
                  <div>
                    <h3 className="font-display font-bold text-red-300">EMERGENCY LOCKDOWN ACTIVE: MAINTENANCE MODE ON</h3>
                    <p className="mt-1 text-xs text-red-300/80 leading-relaxed">
                      All normal users are blocked from access. Only owner accounts and administrators retain database bypass permissions. Remember to toggle Maintenance Mode off from the Platform Control panel or Emergency Center when service is restored.
                    </p>
                  </div>
                </div>
              )}
                ...
                <Panel title="Platform Health Quick View">
                  <div className="space-y-3.5">
                    <div className="flex items-center justify-between border-b border-border/40 pb-2.5">
                      <span className="text-xs text-muted-foreground">Database Sync Integration</span>
                      <Badge tone={healthStatus.db === "ok" ? "win" : "loss"}>{healthStatus.db === "ok" ? "ONLINE" : "OFFLINE"}</Badge>
                    </div>
                    <div className="flex items-center justify-between border-b border-border/40 pb-2.5">
                      <span className="text-xs text-muted-foreground">Vercel Edge Functions API</span>
                      <Badge tone={healthStatus.api === "ok" ? "win" : "loss"}>{healthStatus.api === "ok" ? "ONLINE" : "OFFLINE"}</Badge>
                    </div>
                    <div className="flex items-center justify-between border-b border-border/40 pb-2.5">
                      <span className="text-xs text-muted-foreground">AI Inference Pipeline (Groq)</span>
                      <Badge tone={healthStatus.ai === "ok" ? "win" : "loss"}>{healthStatus.ai === "ok" ? "READY" : "UNCONFIGURED"}</Badge>
                    </div>
                    <div className="flex items-center justify-between pb-1">
                      <span className="text-xs text-muted-foreground">SMTP/Resend Mail Dispatcher</span>
                      <Badge tone={healthStatus.email === "ok" ? "win" : "loss"}>{healthStatus.email === "ok" ? "CONFIGURED" : "OFFLINE"}</Badge>
                    </div>
                    <button
                      onClick={() => { runHealthDiagnostic(); toast.info("Diagnostic health check triggered."); }}
                ...
                <Panel title="Platform Access Controls Summary">
                  <div className="space-y-3 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className={registrationEnabled ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>{registrationEnabled ? "YES" : "NO"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Logins Allowed:</span>
                      <span className={loginEnabled ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>{loginEnabled ? "YES" : "NO"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">AI Coach Active:</span>
                      <span className={aiEnabled ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>{aiEnabled ? "YES" : "NO"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">MetaTrader 5 Sync:</span>
                      <span className={mt5Enabled ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>{mt5Enabled ? "YES" : "NO"}</span>
                    </div>
                    <div className="mt-4 border-t border-border/40 pt-4 flex gap-2">
                      <button onClick={() => setActiveTab("platform")} className="flex-1 text-center py-2 bg-primary/10 text-primary border border-primary/20 rounded-xl hover:bg-primary/20 transition font-semibold">
                        Edit Controls
                      </button>
                      <button onClick={() => setActiveTab("emergency")} className="flex-1 text-center py-2 bg-red-500/10 text-red-300 border border-red-500/20 rounded-xl hover:bg-red-500/20 transition font-semibold">
                        Emergency
                      </button>
                    </div>
                  </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250 flex-1 flex flex-col">
                    ...
                    {(["all", "pending", "approved", "rejected", "suspended"] as const).map((f) => (
                      <button
                        ...
                        className={`rounded-lg px-2.5 py-1 text-[11px] capitalize transition ${
                          userFilter === f ? "bg-primary/20 text-primary font-semibold" : "text-muted-foreground hover:text-foreground"
                        }`}
                      >
                        {f}
                      </button>
              ...
              >
                ...
                {isFetchingUsers ? (
                  ...
                  <p className="py-8 text-center text-xs text-muted-foreground">No users matching search or filter.</p>
                ) : (
                  <div className="overflow-x-auto flex-1">
                    <table className="w-full min-w-[620px] text-sm">
                      ...
                      <tbody>
                        {filteredUsers.map((u) => (
                          <tr key={u.id} className="border-t border-border/40 transition hover:bg-muted/30">
                            <td className="py-3">
                              <button
                              ...
                              >
                                ...
                                <div>
                                  <p className="font-medium text-foreground group-hover:text-primary transition">{u.full_name || "Trader"}</p>
                  ...
                  <div className="w-full max-w-md h-full bg-card border-l border-border p-6 shadow-2xl overflow-y-auto flex flex-col justify-between animate-in slide-in-from-right duration-350 relative">
                    ...
                    <div>
                      ...
                      <div className="mt-6 flex flex-col items-center text-center">
                        ...
                        <h3 className="mt-4 font-display text-lg font-semibold">{selectedUser.full_name || "Trader"}</h3>
                        <p className="text-xs text-muted-foreground">{selectedUser.email}</p>
                        
                        <div className="mt-3 flex gap-2">
                          <Badge tone={selectedUser.status === "approved" || selectedUser.is_owner ? "win" : selectedUser.status === "pending" ? "primary" : selectedUser.status === "suspended" ? "loss" : "muted"}>
                            {selectedUser.status}
                          </Badge>
                      ...
                      <div className="mt-8 space-y-4 text-xs">
                        <div className="flex justify-between border-b border-border/40 pb-2.5">
                          ...
                          <span className="font-mono text-[10px] select-all">{selectedUser.id}</span>
                        </div>
                        <div className="flex justify-between border-b border-border/40 pb-2.5">
                    ...
                    <div className="mt-8 border-t border-border/40 pt-4 space-y-3">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Administrative Actions</p>
                      ...
                      {selectedUser.is_owner || selectedUser.email === "nakulrathi641@gmail.com" ? (
                        <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-[11px] text-primary leading-relaxed">
                          🛡️ This account is flagged as the site Owner. Demoting, suspending, or deleting the primary owner from the UI is locked by core security configurations.
                        </div>
                      ) : (
          ...
          {activeTab === "approvals" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250 flex-1 flex flex-col">
              <Panel title="Access Request Pipeline">
                {isFetchingUsers ? (
                  <p className="py-8 text-center text-xs text-muted-foreground">Scanning pipeline...</p>
                  ...
                  <div className="space-y-3.5">
                    {usersList.filter(u => u.status === "pending").map(u => (
                      <div key={u.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-border/50 bg-card/40 rounded-2xl gap-3">
                        <div>
                          <p className="font-semibold text-sm text-foreground">{u.full_name || "New Trader"}</p>
                          <p className="text-xs text-muted-foreground">{u.email}</p>
                          <p className="text-[10px] text-muted-foreground/60 font-mono mt-0.5">Requested {new Date(u.created_at).toLocaleString()}</p>
                        </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">
                        Blocks all standard user operations and redirects to upgrades screen. Owner retains bypass privileges.
                      </p>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">
                        Toggles registration flow. If turned off, new signup attempts are blocked at authentication fallback.
                      </p>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">
                        Blocks logging in for normal accounts. Toggling this off forces active users out immediately.
                      </p>
                    </div>
                    <input
                      type="checkbox"
              ...
              <Panel title="Centralized Feature Flag Controls">
                <div className="space-y-5">
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Toggle individual feature blocks system-wide. Disabling a feature removes/hides interface access points in client side routing, preserving all database tables and user rows safely.
                  </p>

                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      <p className="text-xs font-semibold flex items-center gap-1.5"><Brain className="size-3.5 text-primary" /> AI Coach Module</p>
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">Enable the Groq AI trading psychology coach interface.</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={aiEnabled}
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      <p className="text-xs font-semibold flex items-center gap-1.5"><SlidersHorizontal className="size-3.5 text-primary" /> MT5 Integration</p>
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">Enable MetaTrader 5 API synchronization and license keys dashboard.</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={mt5Enabled}
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      <p className="text-xs font-semibold flex items-center gap-1.5"><FileText className="size-3.5 text-primary" /> Trading Journal Features</p>
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">Enforces write permissions on journals. Disabling locks entries as read-only.</p>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-4">
                    <div>
                      <p className="text-xs font-semibold flex items-center gap-1.5"><Activity className="size-3.5 text-primary" /> Live Market Data feeds</p>
                      <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm mt-0.5">Sync and display active market data feeds and indexes.</p>
                    </div>
                    <input
                      type="checkbox"
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250">
              <Panel title="Schedule Rollout & Maintenance Deadlines">
                <form onSubmit={handleSaveDeadline} className="grid gap-4 sm:grid-cols-2 text-xs">
                  <div>
                    <label className="block text-muted-foreground font-semibold">Feature / Rollout Label</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-muted-foreground font-semibold">Rollout Status</label>
                    <select
                  ...
                  <div>
                    <label className="block text-muted-foreground font-semibold">Start Timestamp</label>
                    <input
                      type="datetime-local"
                  ...
                  <div>
                    <label className="block text-muted-foreground font-semibold">Target Completion Deadline</label>
                    <input
                      type="datetime-local"
              ...
              <Panel title="Active System Deadlines & Rollouts">
                {isFetchingDeadlines ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">Scanning database schedules...</p>
                ) : deadlines.length === 0 ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">No active deadlines logged in database.</p>
                ) : (
                  <div className="space-y-3.5">
                    {deadlines.map((dl) => {
                      const hasEnded = dl.deadline ? new Date() > new Date(dl.deadline) : false;
                      return (
                        <div key={dl.id} className="flex justify-between items-center p-4 border border-border/50 bg-card/40 rounded-2xl text-xs">
                          <div>
                            <p className="font-semibold text-sm text-foreground">{dl.feature_name}</p>
                          ...
                          <div className="flex items-center gap-3">
                            <Badge tone={dl.status === "completed" ? "win" : dl.status === "active" ? "primary" : dl.status === "maintenance" ? "loss" : "muted"}>
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between p-4 border border-border/40 bg-card/30 rounded-2xl gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl border ${healthStatus.auth === "ok" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : healthStatus.auth === "checking" ? "bg-primary/10 border-primary/20 text-primary" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                        <Key className="size-5" />
                      </div>
                      <div>
                        <p className="text-xs font-semibold">Supabase Auth System</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">Provides OTP verification and JWT session tokens.</p>
                      </div>
                    </div>
                    <Badge tone={healthStatus.auth === "ok" ? "win" : healthStatus.auth === "checking" ? "primary" : "loss"}>
                  ...
                  <div className="flex items-start justify-between p-4 border border-border/40 bg-card/30 rounded-2xl gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl border ${healthStatus.db === "ok" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : healthStatus.db === "checking" ? "bg-primary/10 border-primary/20 text-primary" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                        <FileText className="size-5" />
                      </div>
                      <div>
                        <p className="text-xs font-semibold">Supabase PostgreSQL DB Connection</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">Core persistence storage layer for trades, settings, and profiles.</p>
                      </div>
                    </div>
                    <Badge tone={healthStatus.db === "ok" ? "win" : healthStatus.db === "checking" ? "primary" : "loss"}>
                  ...
                  <div className="flex items-start justify-between p-4 border border-border/40 bg-card/30 rounded-2xl gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl border ${healthStatus.api === "ok" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : healthStatus.api === "checking" ? "bg-primary/10 border-primary/20 text-primary" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                        <SlidersHorizontal className="size-5" />
                      </div>
                      <div>
                        <p className="text-xs font-semibold">Vercel API Serverless Functions</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">Executes backend routines, notifications, and analytics controllers.</p>
                      </div>
                    </div>
                    <Badge tone={healthStatus.api === "ok" ? "win" : healthStatus.api === "checking" ? "primary" : "loss"}>
                  ...
                  <div className="flex items-start justify-between p-4 border border-border/40 bg-card/30 rounded-2xl gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl border ${healthStatus.ai === "ok" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : healthStatus.ai === "checking" ? "bg-primary/10 border-primary/20 text-primary" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                        <Brain className="size-5" />
                      </div>
                      <div>
                        <p className="text-xs font-semibold">Groq AI Inference Engine</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">Calculates emotional grading and provides psychology feedback.</p>
                      </div>
                    </div>
                    <Badge tone={healthStatus.ai === "ok" ? "win" : healthStatus.ai === "checking" ? "primary" : "loss"}>
                      {healthStatus.ai === "ok" ? "READY" : healthStatus.ai === "checking" ? "SCANNING" : "UNCONFIGURED"}
                    </Badge>
                  </div>
                  ...
                  <div className="flex items-start justify-between p-4 border border-border/40 bg-card/30 rounded-2xl gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl border ${healthStatus.email === "ok" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : healthStatus.email === "checking" ? "bg-primary/10 border-primary/20 text-primary" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                        <Mail className="size-5" />
                      </div>
                      <div>
                        <p className="text-xs font-semibold">SMTP/Resend Mail Dispatcher</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">Sends status approval alerts and OTP recovery links.</p>
                      </div>
                    </div>
                    <Badge tone={healthStatus.email === "ok" ? "win" : healthStatus.email === "checking" ? "primary" : "loss"}>
                      {healthStatus.email === "ok" ? "ONLINE" : healthStatus.email === "checking" ? "SCANNING" : "OFFLINE"}
                    </Badge>
                  </div>

                  {healthStatus.lastChecked && (
                    <p className="text-[10px] text-muted-foreground text-center mt-3">Last health scan completed at: {healthStatus.lastChecked}</p>
                  )}
                </div>
              </Panel>
          ...
          {activeTab === "security" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250 flex-1 flex flex-col">
              <Panel
                ...
                action={
                  <button
                    onClick={() => { fetchAuditLogs(); toast.success("Logs list synchronized."); }}
              ...
              >
                <p className="text-xs text-muted-foreground mb-4">
                  Immutable administrative log showing target mutation events, states, administrator emails, and execution results. Safe details are presented (secrets and tokens are masked).
                </p>

                {isFetchingLogs ? (
                  <p className="py-8 text-center text-xs text-muted-foreground">Scanning audit log index…</p>
                ) : auditLogs.length === 0 ? (
                  <p className="py-8 text-center text-xs text-muted-foreground">No administrative actions logged in system.</p>
                ) : (
                  <div className="overflow-x-auto flex-1">
                    <table className="w-full min-w-[650px] text-xs">
                      ...
                      <tbody>
                        {auditLogs.map((log) => (
                          <tr key={log.id} className="border-t border-border/40 transition hover:bg-muted/20">
                            <td className="py-2.5 font-mono text-[10px] text-muted-foreground">
                              {new Date(log.timestamp).toLocaleString()}
                            </td>
                            <td className="py-2.5 font-medium text-foreground">{log.admin_email}</td>
                            <td className="py-2.5 font-mono text-[10px] text-primary">{log.action}</td>
                            <td className="py-2.5 text-muted-foreground">
                              {log.target_type} {log.target_id ? `(${log.target_id.slice(0, 8)})` : ""}
                            </td>
                            <td className="py-2.5 text-right">
                              <Badge tone={log.result === "success" ? "win" : "loss"}>{log.result.toUpperCase()}</Badge>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250">
              <Panel title="Draft & Publish Targeted Announcement">
                <form onSubmit={handleSaveAnnouncement} className="space-y-4 text-xs">
                  <div>
                    <label className="block text-muted-foreground font-semibold">Announcement Title</label>
                    <input
                      type="text"
                      value={newAnnTitle}
                  ...
                  <div>
                    <label className="block text-muted-foreground font-semibold">Notification Content Message</label>
                    <textarea
                      value={newAnnMessage}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="block text-muted-foreground font-semibold">Audience Scope</label>
                      <select
                    ...
                    <div>
                      <label className="block text-muted-foreground font-semibold">Initial Status</label>
                      <select
                        value={newAnnStatus}
              ...
              <Panel title="Platform News & Announcement History">
                {isFetchingAnnouncements ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">Retrieving platform history...</p>
                ) : announcements.length === 0 ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">No announcement entries discovered.</p>
                ) : (
                  <div className="space-y-3.5">
                    {announcements.map((ann) => (
                      <div key={ann.id} className="p-4 border border-border/50 bg-card/40 rounded-2xl text-xs space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-sm text-foreground">{ann.title}</h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">
                              Created {new Date(ann.created_at).toLocaleDateString()} · Target: <span className="font-semibold text-primary capitalize">{ann.audience}</span>
                            </p>
                          </div>
                        ...
                        <p className="text-muted-foreground leading-relaxed text-xs">{ann.message}</p>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250">
              <Panel title="SMTP & Email Dispatch Diagnostic Center">
                <form onSubmit={handleSendTestEmail} className="space-y-4 text-xs">
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Verify connection to SMTP/Resend on-demand. Enter your target email to dispatch a mock Account Approval/Decline template notification. No data records will mutate.
                  </p>

                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="block text-muted-foreground font-semibold">Recipient Name</label>
                      <input
                        type="text"
                        value={testEmailName}
                    ...
                    <div>
                      <label className="block text-muted-foreground font-semibold">Destination Email</label>
                      <input
                        type="email"
                  ...
                  <div>
                    <label className="block text-muted-foreground font-semibold">Mock Access Action Status</label>
          ...
          {activeTab === "emergency" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-250">
              ...
              <Panel title="Emergency Maintenance Mode Lock" className="border-red-500/30 bg-red-500/5 relative overflow-hidden">
                ...
                <div className="space-y-4 text-xs">
                  <div className="flex gap-3.5 items-start">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-red-300">Lock Entire Application Under Maintenance</h3>
                      <p className="mt-1 text-muted-foreground leading-relaxed">
                        Redirects all standard traders instantly to upgrades/maintenance screen. Admins and owners bypass this lock automatically.
                      </p>
                    </div>
                  </div>

                  <div className="border-t border-red-500/20 pt-4 space-y-3">
                    <label className="block text-red-200">To confirm, type <span className="font-mono font-bold text-white bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30">MAINTENANCE</span> below:</label>
                    <input
                      type="text"
                    ...
                    {maintenance ? (
                      ...
                      >
                        <Lock className="inline size-4 mr-1.5 animate-pulse" /> Activate Maintenance Mode
                      </button>
                    )}
                  </div>
                </div>
              ...
              <Panel title="Emergency Registration Halt" className="border-red-500/30 bg-red-500/5 relative overflow-hidden">
                <div className="pointer-events-none absolute -left-20 -top-20 size-80 rounded-full bg-red-500/10 blur-3xl" />
                <div className="space-y-4 text-xs">
                  <div className="flex gap-3.5 items-start">
                    <Ban className="size-10 shrink-0 text-red-400" />
                    <div>
                      <h3 className="text-sm font-bold text-red-300">Disable New Registrations Immediately</h3>
                      <p className="mt-1 text-muted-foreground leading-relaxed">
                        Blocks anyone from submitting OTP signup queries. Prevents spam registry attacks. Active logins are unaffected.
                      </p>
                    </div>
                  </div>

                  <div className="border-t border-red-500/20 pt-4 space-y-3">
                    <label className="block text-red-200">To confirm, type <span className="font-mono font-bold text-white bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30">HALT REGISTRATION</span> below:</label>
                    <input
                      type="text"
              ...
              <Panel title="Emergency Authentication Halt" className="border-red-500/30 bg-red-500/5 relative overflow-hidden">
                <div className="pointer-events-none absolute -left-20 -top-20 size-80 rounded-full bg-red-500/10 blur-3xl" />
                <div className="space-y-4 text-xs">
                  <div className="flex gap-3.5 items-start">
                    <ShieldAlert className="size-10 shrink-0 text-red-400" />
                    <div>
                      <h3 className="text-sm font-bold text-red-300">Disable Authentication Services Immediately</h3>
                      <p className="mt-1 text-muted-foreground leading-relaxed">
                        Blocks all standard user authentications. Actively logged in traders are forced out. Owner is protected and retains emergency dashboard access.
                      </p>
                    </div>
                  </div>

                  <div className="border-t border-red-500/20 pt-4 space-y-3">
                    <label className="block text-red-200">To confirm, type <span className="font-mono font-bold text-white bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30">SHUTDOWN</span> below:</label>
                    <input
                      type="text"
              ...
              <Panel title="Commit Emergency Actions">
                <div className="space-y-4 text-xs">
                  <p className="text-muted-foreground leading-relaxed">
                    ⚠️ Emergency actions toggled above will NOT persist or take effect until committed. Confirm setting choices below.
                  </p>
                  <button
                    onClick={handleSavePlatformSettings}
                    disabled={isSavingSettings}
                  ...
                  >
                    <Save className="size-3.5" />
                    {isSavingSettings ? "Saving system settings…" : "Commit Platforms and Save Emergency Configuration"}
                  </button>
                </div>
              </Panel>
