...
type MarketResponse = {
  ...
  data?: {
    ...
    lastUpdated: string;
    marketStatus: "LIVE" | "CLOSED" | "PRE-OPEN";
  };
};

export function IndianMarketLive() {
  ...
  if (!market) {
    return (
      <Panel title="Indian Market Live" className="h-full flex flex-col justify-center">
  ...
  if (market.status !== "live" || !market.data) {
    return (
      <Panel title="Indian Market Live" className="h-full flex flex-col">
    ...
    >
      <div className="grid grid-cols-2 gap-3 mt-4">
        {market.data.instruments.map((inst) => {
          ...
          return (
            <div key={inst.symbol} className="rounded-xl border border-border/40 bg-card/40 p-3 sm:p-4 transition hover:bg-card/70">
              <p className="text-[11px] sm:text-xs font-semibold text-muted-foreground tracking-wide uppercase">{inst.symbol}</p>
              <p className="font-display text-lg sm:text-xl font-bold text-foreground mt-1 tracking-tight">
                {inst.price.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </p>
              <div className="flex items-center gap-2 mt-2">
                <span className={`text-[10px] sm:text-xs font-bold px-1.5 py-0.5 rounded ${bgClass} ${colorClass}`}>
                  {sign}{inst.change.toFixed(2)}
                </span>
