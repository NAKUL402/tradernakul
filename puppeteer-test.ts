...
(async () => {
  ...
  try {
    ...
    console.log(
      ...
      htmlSettings.includes("Settings") && !page.url().includes("/login"),
    );
    ...
    console.log(
      ...
      htmlAdmin.includes("Admin") && !page.url().includes("/login"),
    );
    ...
    const hasTestOwner = await page.evaluate(() => {
      return (
        document.body.innerText.includes("Test Owner") ||
        document.body.innerText.includes("test-owner@local.test")
      );
    });
