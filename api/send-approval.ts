 ...
 *
 * Sends approval request email to the owner (nakultrader007@gmail.com).
 * Uses Resend API first, falls back to Gmail SMTP.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log(`[send-approval] Sending approval email to owner: ${ownerEmail} for user: ${userEmail}`);

  const htmlBody = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #0b0c16; color: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid rgba(99,102,241,0.2);">
      ...
      <div style="padding: 32px 40px;">
        ...
        <div style="text-align: center;">
          <a href="${approveLink}" style="display: inline-block; background: #059669; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 10px; font-weight: 700; font-size: 15px; margin-right: 12px;">✅ Approve</a>
          <a href="${rejectLink}" style="display: inline-block; background: #dc2626; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 10px; font-weight: 700; font-size: 15px;">❌ Reject</a>
        </div>
        <p style="font-size: 11px; color: #475569; text-align: center; margin: 20px 0 0;">You can also manage users from the <a href="${baseUrl}/admin" style="color: #6366f1;">Admin Panel</a>.</p>
      </div>
    </div>
  `;
  ...
  if (resendApiKey) {
    try {
      ...
      if (!response.ok) {
        const errMsg = data?.message || data?.error || `Resend error: ${response.status}`;
        console.error("[send-approval] Resend failed:", errMsg);
        return res.status(500).json({ success: false, error: errMsg });
      }

      console.log(`[send-approval] ✅ Approval email sent via Resend to owner: ${ownerEmail}`);
  ...
  if (smtpUser && smtpPass) {
    try {
      ...
      await transporter.verify().catch((e: Error) => {
        throw new Error(`SMTP auth failed: ${e.message}`);
      });
      await transporter.sendMail({
        ...
        html: htmlBody,
        text: `Approve: ${approveLink}\nReject: ${rejectLink}`,
      });
      console.log(`[send-approval] ✅ Approval email sent via Gmail SMTP to: ${ownerEmail}`);
      return res.status(200).json({ success: true });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error("[send-approval] Gmail SMTP error:", msg);
      return res.status(500).json({ success: false, error: `SMTP error: ${msg}` });
    }
  }

  const missingMsg = "Email not configured. Set RESEND_API_KEY or EMAIL_USER + EMAIL_PASS in Vercel environment.";
  console.error("[send-approval] FATAL:", missingMsg);
